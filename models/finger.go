...
func ProcessURL(url string) {
	...
	// 用于保存主URL信息的变量
	var mainTitle, mainServer string
	var mainStatusCode int
	var mainInfoSet bool
	var mainInfoMu sync.Mutex

	wg.Add(2)
	go processWithInfo(url, nil, resultsChannel, &matchedCMS, &mu, &wg, &errOccurred, &mainTitle, &mainServer, &mainStatusCode, &mainInfoSet, &mainInfoMu)
	...
	randomURL := url + suffix
	go process(randomURL, nil, resultsChannel, &matchedCMS, &mu, &wg, &errOccurred)
	wg.Wait()
	...
	if countItems(&matchedCMS) == 0 && !errOccurred {
		mainInfoMu.Lock()
		if mainInfoSet {
			color.White("[%s] [*] [%s] [Not Matched] [%d] [%s] [%s]", time.Now().Format("01-02 15:04:05"), url, mainStatusCode, mainServer, mainTitle)
		}
		mainInfoMu.Unlock()
	}
}
